...
const exitFormModal = (evt) => {
    if(evt.target.id == "exit-modal"){
        setModal(false)
    }
}
  ...
  return (
    <div onClick={exitBtnClick} id="modal-wrapper" className={` transition-[0.4s] modal ${modal ? "" : "scale-0"}`}>
     <div className="modal-content relative">
        <button onClick={exitFormModal} id="exit-modal" className={`absolute right-[10px] top-[10px] transition-[0.4s] ${modal ? "" : "scale-0"}`}>
